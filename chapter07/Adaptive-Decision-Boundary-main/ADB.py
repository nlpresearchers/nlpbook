...
class ModelManager:
    ...
    def train(self, args, data):     
        ...
        for i, j in enumerate(criterion_boundary.parameters()):
            print(i, j)
        ...
        # torch.save(self.centroids, "./myTensor.pt")
        self.centroids = torch.load("./myTensor.pt")
        wait = 0
    ...
    def centroids_cal(self, args, data):
        centroids = torch.zeros(data.num_labels, args.feat_dim)
        total_labels = torch.empty(0, dtype=torch.long).to(self.device)
        with torch.set_grad_enabled(False):
