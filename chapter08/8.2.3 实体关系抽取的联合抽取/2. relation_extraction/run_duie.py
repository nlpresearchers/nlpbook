...
parser.add_argument("--pretrained_model_path", default='bert-base-chinese', type=str)
parser.add_argument("--embedding_dim", default=768, type=int)
parser.add_argument("--hidden_dim", default=128, type=int)
parser.add_argument("--tagset_size", default=0, type=int)
...
class BertRE(nn.Layer):
    def __init__(self, hparams):
        super(BertRE, self).__init__()
        # 加载已下载的 BERT 模型或重新训练 BERT 模型
        self.pretrained_model_path = hparams.pretrained_model_path or 'bert-base-chinese'
        self.embedding_dim = hparams.embedding_dim
        self.hidden_dim = hparams.hidden_dim
        self.tagset_size = hparams.tagset_size  # 标签类别数
        self.bert_model = BertModel.from_pretrained(self.pretrained_model_path)
        self.en_bilstm = nn.LSTM(self.embedding_dim,self.hidden_dim//2, direction='bidirectional')
        self.pool = nn.Linear(self.hidden_dim,self.tagset_size)
        self.log_softmax = nn.LogSoftmax(axis=-1)

    def forward(self, input_ids):
        sequence_output, pooled_output = self.bert_model(input_ids=input_ids)
        out, (h_n, c_n) = self.en_bilstm(sequence_output, None)
        out = self.pool(out)
