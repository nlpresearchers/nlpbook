...
class Model_data_preparation(object):
    ...
    def __init__(self, DATA_DIR="raw_data", DATA_OUTPUT_DIR="tokened_labeling", spo_list_separator='[SPO_SEP]',
    ...
    def generate_Entity_name_slipe_by_type(self):
        pass

    # 生成训练数据和验证数据中所有实体名称的文件
    def generate_Entity_name_dict(self):
        entity_name_set = set()
        #检查括号不对称的 spo 三元组
        ...
        with open(os.path.join(self.DATA_DIR, "Entity_name_dict.txt"), "w", encoding='utf-8') as f:
            for name in entity_names_list:
                f.write(name + "\n")
            print("Entity_name_dict.txt is saved to {}".format(os.path.join(self.DATA_DIR, "Entity_name_dict.txt")))
...
if __name__=="__main__":
    ...
    DATA_OUTPUT_DIR = "data/corrected3"
